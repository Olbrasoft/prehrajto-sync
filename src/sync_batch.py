...
Pro každý film: pick → resolve_cdn → curl download → upload → save_state →
mazání tmp. State + log se zapisují **po každém filmu**, takže timeout/crash
neztratí progress.
...
from prehrajto_upload import login, upload_video
...
def download(url: str, dest: Path, timeout_sec: int = 540) -> int:
    ...
    cmd = [
        ...
        "-s", "-S",  # silent kromě errorů — log obstará náš code
        "-o", str(dest),
    ]
...
def process_one(film: dict, session, state: dict) -> bool:
    ...
    # 1. CDN resolve
    t = time.monotonic()
    log(f"step=cdn-resolve placeholder={film['url']}")
    resolved = resolve_cdn(film["url"])
    cdn_sec = round(time.monotonic() - t, 1)
    if not resolved:
        log(f"step=cdn-resolve FAILED cr_film_id={cr_film_id} after {cdn_sec}s")
    ...
    try:
        ...
        log(f"step=download done cr_film_id={cr_film_id} size={size} "
            f"dur={dl_sec}s speed={mbps}MB/s")
    except Exception as e:
        dl_sec = round(time.monotonic() - t, 1)
        log(f"step=download FAILED cr_film_id={cr_film_id} after {dl_sec}s err={e}")
        if tmp_path.exists():
            tmp_path.unlink()
    ...
    t = time.monotonic()
    try:
        log(f"step=upload start cr_film_id={cr_film_id} size={size}")
        ...
        log(f"step=upload done cr_film_id={cr_film_id} video_id={video_id} "
            f"dur={up_sec}s speed={up_mbps}MB/s")
    except Exception as e:
        up_sec = round(time.monotonic() - t, 1)
        log(f"step=upload FAILED cr_film_id={cr_film_id} after {up_sec}s err={e}")
    ...
    log(f"step=film done cr_film_id={cr_film_id} total={total_sec}s "
        f"(cdn={cdn_sec}s + download={dl_sec}s + upload={up_sec}s)")

    # 4. State append + persist
    state["uploads"].append({
        "cr_film_id": cr_film_id,
        "cr_slug": film["cr_slug"],
        "title": film["title"],
        "year": film["year"],
        "sktorrent_id": film["id"],
    ...
    save_state(state)
    log(f"step=state-saved cr_film_id={cr_film_id} (total uploads={len(state['uploads'])})")
    return True
...
def main() -> int:
    ...
    succeeded = 0
    failed = 0
    extra_exclude: set[int] = set()  # filmy, co v této dávce selhaly nebo už proběhly
    ...
    for i in range(args.count):
        ...
        if film is None:
            ...
            break
        # ihned vyloučit z dalších iterací (i kdyby selhal — nezkoušíme dvakrát v jednom runu)
